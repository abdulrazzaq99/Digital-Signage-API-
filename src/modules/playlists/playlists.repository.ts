...
export const playlistsRepository = {
   ...
   */
  syncItems: async (playlistId: string, items: { id?: string; assetId: string; durationSec: number }[], tx: Tx) => {
    ...
    for (const [position, it] of items.entries()) {
      if (it.id) await tx.playlistItem.update({ where: { id: it.id }, data: { position, durationSec: it.durationSec, assetId: it.assetId } });
      else await tx.playlistItem.create({ data: { playlistId, position, assetId: it.assetId, durationSec: it.durationSec } });
    }
  },
  assetsInCompany: (companyId: string, ids: string[], tx?: Tx) => (tx ?? prisma).mediaAsset.findMany({ where: { companyId, id: { in: ids }, status: "READY" }, select: { id: true, type: true } }),
